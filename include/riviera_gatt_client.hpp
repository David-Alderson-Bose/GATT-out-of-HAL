 ...
 * @param exact_match: If false, will connect to the first found device that
 * starts with 'name'
 ...
 */
ConnectionPtr Connect(std::string name, bool exact_match = false,
                      int timeout = -1);
...
public:
     ...
     * @param timeout: miliseconds to wait before returning. Set to zero to wait
     * indefinitely.
     * @return: 0 if returning on connection available, non-zero if returning on
     * timeout
     */
...
#define WRITE_TYPE(x) x,
    ...
    int WriteCharacteristic(RivieraBT::UUID, std::string to_write,
                            WriteType type = COMMAND, int index=0);
     ...
     * @return: Non-zero on faulure, zero on success
     */
    int WriteCharacteristicWhenAvailable(RivieraBT::UUID, std::string to_write,
                                         WriteType type = COMMAND,
     ...
     */
    std::string ReadCharacteristic(RivieraBT::UUID uuid, int index=0);
     ...
     */
    int ReadCharacteristic(RivieraBT::UUID uuid, ReadCallback cb, int index=0);
     ...
     * @return: zero on success, non-zero on failure
     */
    int ReadCharacteristicWhenAvailable(RivieraBT::UUID uuid, ReadCallback cb,
                                        unsigned int timeout = 0);
     ...
     * @return: zero on success, non-zero on failure
     */
    int RegisterNotification(RivieraBT::UUID uuid, int index=0);
    int DeregisterNotification(RivieraBT::UUID uuid, int index=0);
...
protected:
    Connection(std::string name, int conn_id, bt_bdaddr_t* bda,
               ConnectionData* data);
